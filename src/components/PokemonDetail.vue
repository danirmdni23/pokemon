        ...
        <div class="flex justify-center items-center px-2 py-2 lg:px-4 lg:py-7">
          <div class="w-full">
            ...
            <div class="overflow-y-auto no-scrollbar h-52 lg:h-80 lg:mt-5 mt-2 sm:mt-3 w-full justify-center items-center">
              <div class="bg-slate-300 my-2 rounded-lg flex w-full px-4 py-2 gap-2" style="background-color: #e2e8f0bf;" v-for="(pcn, index) in pCatchName" :key="pcn" v-if="pCatchName, pCatchId, pCatchImg">
                <div>
...
const catchPokemon = async () => {
  ...
  pCatchName.value = JSON.parse(localStorage.getItem("pokemonName"));
  pCatchId.value   = JSON.parse(localStorage.getItem("pokemonId"));
  pCatchImg.value  = JSON.parse(localStorage.getItem("pokemonImg"));
};
