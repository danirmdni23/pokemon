...
import PokemonDetailApi from '../../apis/PokemonDetailApi';
...
vi.mock('../../apis/PokemonDetailApi.js');
...
describe('PokemonDetail.vue', () => {
    ...
    it('fetches and renders Pokémon data', async () => {
        ...
        wrapper.vm.fetchPokemon()
        await flushPromises();
        const pokemonName = wrapper.find('[data-test="pokemon-name"]');
    ...
    it.only('catch Pokemon', async () => {
        const wrapper = mount(PokemonDetail, { global: { stubs: ['PokemonModal'] } });
        wrapper.vm.fetchPokemon()
        await flushPromises();
        const pokemonCatch = wrapper.find('[data-test="pokemon-catch"]');
        await pokemonCatch.trigger('click');
        await flushPromises();
        const resultPokemon = wrapper.find('[data-test="result-pokemon"]');
        expect(resultPokemon.text()).toContain('1');
    });
...
function mockSuccess() {
    PokemonDetailApi.get.mockResolvedValue({
        data: {
...
function mockError() {
    PokemonDetailApi.get.mockRejectedValue(new Error('Failed to fetch Pokémon data'));
}
